...
export default function EditEntry({ params }) {
  ...
  const fetchEntry = async () => {
    try {
      const data = await apiFetch(`/journal/id/${id}`);
      setTitle(data.title || "");
      setContent(data.content || "");
    } catch (err) {
      setError("Failed to load entry. It might have been deleted.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white p-6 selection:bg-purple-500/30">
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-8 sm:p-10 shadow-2xl">
          <h1 className="text-3xl font-bold mb-8 text-white">Edit Entry</h1>
            ...
            <div className="pt-4 flex justify-end">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Save className="w-5 h-5" /> Update Entry
                  </>
                )}
              </button>
            </div>
